...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> saveDataToFirestore(User currentUser, String imageUrl) async {
    // Use name as initial shop name
    final shopName = nameFieldController.text.trim();
    ...
    await FirebaseFirestore.instance
      ...
      // Shop details - initially same as seller details
      "shopName": shopName,
      "shopImage": imageUrl,
      "status": "approved",
      "earnings": 0.0,
    ...
    await sharedPreferences!
        .setString("sellerEmail", currentUser.email!.toString());
    await sharedPreferences!
        .setString("sellerName", nameFieldController.text.trim());
    await sharedPreferences!.setString("sellerAvatarUrl", imageUrl);
    await sharedPreferences!
        .setString("phone", phoneFieldController.text.trim());
    await sharedPreferences!
        .setString("address", addressFieldController.text.trim());
    await sharedPreferences!.setString("shopName", shopName);
    await sharedPreferences!.setString("shopImage", imageUrl);
